## Hello, I'm Sai Omkar Attuluri 👋

I am a motivated and detail-oriented Computer Science student with a focus on building, deploying, and securing production-grade web applications. I am a recent B.Tech graduate in Computer Science & Engineering(2022-2026).

I specialize in creating end-to-end RESTful API solutions and am a **ServiceNow Certified System Administrator (CSA) & Certified Application Developer (CAD)**.

---

### 🛠 Technical Toolbox

*   **Languages:** Java, Python, JavaScript
*   **Frontend:** React.js, Vite, HTML5, CSS3
*   **Backend:** Node.js, Express.js, RESTful APIs, JWT Authentication
*   **Databases:** MongoDB Atlas, MySQL
*   **Cloud & DevOps:** Netlify, Render, Git, GitHub, CI/CD
*   **Platforms:** ServiceNow

---

### 🚀 Featured Projects

#### **[ByteBudget](https://bytebudget-tracker.netlify.app/) - Full-Stack Expense Tracker**
*   Developed a production-ready MERN stack application with secure JWT authentication.
*   Architected a RESTful API with 8+ endpoints using MVC patterns.
*   **Tech:** React, Node.js, Express, MongoDB Atlas.

#### **[AtmosVision](https://saiomkar058.github.io/AtmosVision/) - Real-Time Weather App**
*   Integrated OpenWeatherMap API to provide live data for 200,000+ cities globally.
*   Optimized DOM manipulation for faster rendering and seamless user experience.
*   **Tech:** JavaScript, HTML, CSS, OpenWeatherMap API.

---

### 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SaiOmkar058&show_icons=true&theme=tokyonight&count_private=true" alt="Sai Omkar Stats" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=SaiOmkar058&theme=tokyonight" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SaiOmkar058&layout=compact&theme=tokyonight&hide_langs=html,css" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=SaiOmkar058&color=blue&style=flat-square&label=PROFILE+VIEWS" alt="Profile Views" />
</p>

---

### 🏆 Achievements & Certifications
*   **National Hackathon Finalist** (Smart India Hackathon - SIH'24).
*   **ServiceNow:** CSA & CAD Certified.
*   **NPTEL:** Programming in Java.
*   **IBM:** Intro to Software Engineering & Front-End Development.

---

### 📫 Connect with me
*   **LinkedIn:** [linkedin.com/in/attulurisaiomkar](https://linkedin.com/in/attulurisaiomkar)
*   **Email:** [saiomkar475@gmail.com](mailto:saiomkar475@gmail.com)
